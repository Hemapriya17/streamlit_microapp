...
def variable_values(select_name,_col):
    # try:
        with st.form(key='my_form2'):
            ...
            if select_name:
                ...
                doe_name = st.text_input('DOE NAME',key = 'doe_name',value =st.session_state.dict_values_re.get('doe_name',''))
                ...
                num_factors = st.text_input('NUMBER OF FACTORS',key = 'num_factors',value = st.session_state.dict_values_re.get('num_factors','4'))
                ...
                if not num_factors == 'no_value':
                    for i in range(1,int(num_factors)+1):
                        ...
                        with col1:
                            st.text_input(f"Factor # {i} (low,high)", key=f'factor{i}',value = st.session_state.dict_values_re.get(f'factor{i}',''))
                            mongo_data[f'factor{i}'] = st.session_state.get(f'factor{i}','')
                        with col2:
                            st.text_input(f"values # {i}", key=f'values{i}',value = st.session_state.dict_values_re.get(f'values{i}',''))
                            mongo_data[f'values{i}'] = st.session_state.get(f'values{i}','')
                    factor_level = st.text_input('FACTOR LEVEL',key = 'factor_level',value = st.session_state.dict_values_re.get('factor_level',''))
                    mongo_data['factor_level'] = st.session_state.get('factor_level','')
                    print(type(num_factors))
                    print(num_factors)
                    resolution = st.text_input('RESOLUTION',key = 'resolution',value = st.session_state.dict_values_re.get('resolution',''))
                    mongo_data['resolution'] = st.session_state.get('resolution','')
                    num_response = st.text_input('NUMBER OF RESPONSE VARIABLES',key='num_response',value = st.session_state.dict_values_re.get('num_response',''))
                    mongo_data['num_response'] = st.session_state.get('num_response','')
                    response_names = st.text_input('RESPONSE VARIABLE NAMES',key = 'response_names',value = st.session_state.dict_values_re.get('response_names',''))
...
if __name__ == '__main__':
    ...
    try:
        ...
        dict_response,messages = variable_values(select_name,col)
