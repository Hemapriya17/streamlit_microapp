    ...
    # try:
        ...
        if st.session_state['response']:
            ...
            if selection is not None and hasattr(selection, 'selected_rows') and selection.selected_rows:
                ...
                query = f'Extract text under the {test_name.upper()} heading including its sub classes. If the extracted text has referrence to other clause you need to extract those heading/clause text .This must go until all the clause text is extracted from the last extracted text. consider it as reference clause when it has keywords like "see","refer" etc. If any table is refered extract those also.Response should be in tuple where first value should be the text and the second value in the tuple should be the figure name if any figure name is mentioned in the extracted text/ present under the heading. only return the tuple'
                response = chain.run(input_documents=st.session_state['docs'], question=query)
                st.write(response)
                # response = 'FIG. 104'
                ...
                for file in os.listdir():
                    ...
                    file_found = re.findall(r'{}'.format(str(response)),file)
                ...
                selected_case = [Document(page_content=x) for x in text_splitter.split_text(response)]
                st.session_state['selected_case'] = selected_case
                test_plan_query = f'{response} Using the response/text before suggest the following Objective, Apparatus Required, Manufacturers Data Required, Formula, precautions, Pre Test Condition, During Test Condition, Post Test Condition, Measured Value, Success Criteria, Procedure, Circuit Diagram if any, Tabulation and Result order wise. If any base64 string are found then convert it to image and display under the heading "diagram".'
                test_plan = chain.run(input_documents=st.session_state['selected_case'], question=test_plan_query)
                print(test_plan) 
                st.write(test_plan)
            else:
